...
class Bitset {
    ...
    /***
     * This method returns the number of elements in the set, by walking through
     * the byteArray and counting the bits that are set to 1.
     *
    ...
    public int cardinality() {
        ...
        for (int i = 0; i < byteArray.length; i++) {
            count += countBits(byteArray[i]);
        }
        return count;
    }

    /***
     * This method counts the number of bits that are set to 1 in a byte.
     * It does this by shifting the byte right one bit at a time, and checking
     * the least significant bit for a 1. If it is 1, the count is incremented.
     * The byte is shifted right by 1, which is a signed shift, but that is
     * okay here because we are only checking the least significant bit.
     *
     * @param b The byte to count the bits in
    ...
    private int countBits(byte b) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            b >>= 1; // Shift the byte right one bit
        }
        return count;
    }
